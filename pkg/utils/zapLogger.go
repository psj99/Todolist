...
func getWriterSyncer() zapcore.WriteSyncer {
	...
	hook := lumberjack.Logger{
		...
		MaxSize:    config.Cfg.Log.MaxSize,    // 每个日志文件保存500M，默认 100M
		MaxBackups: config.Cfg.Log.MaxBackups, // 保留3个备份，默认不限
		MaxAge:     config.Cfg.Log.MaxAge,     // 保留28天，默认不限
		Compress:   config.Cfg.Log.Compress,   // 是否压缩，默认不压缩
	}
